...
function issuer() {
  ...
  if (!value || !URL.canParse(value))
    throw new Error("COINARC_AUTH_ISSUER must be an absolute URL")
  return value
}
...
export function signJwt(
  ...
  const header = base64url(
    JSON.stringify({ alg: "RS256", typ: "JWT", kid: "coinarc-session-v1" })
  )
...
function walletFromPayload(
  payload: Record<string, unknown>
): WalletClaim | null | undefined {
  const hasWalletClaim =
    "walletAddress" in payload ||
    "walletChainId" in payload ||
    "walletCustody" in payload ||
    "circleWalletId" in payload
  if (!hasWalletClaim) return undefined
  if (
    typeof payload.walletAddress !== "string" ||
    typeof payload.walletChainId !== "number" ||
    !Number.isInteger(payload.walletChainId) ||
    (payload.walletCustody !== "circle" && payload.walletCustody !== "external")
  )
    return null
  if (
    payload.circleWalletId !== undefined &&
    typeof payload.circleWalletId !== "string"
  )
    return null
  if (
    payload.walletCustody === "circle" &&
    typeof payload.circleWalletId !== "string"
  )
    return null
  if (
    payload.walletCustody === "external" &&
    payload.circleWalletId !== undefined
  )
    return null
  return {
    address: payload.walletAddress,
    chainId: payload.walletChainId,
    custody: payload.walletCustody,
    ...(typeof payload.circleWalletId === "string"
...
export function verifyJwt(
  ...
  if (
    ...
    (payload.provider !== "circle" && payload.provider !== "siwe") ||
...
export function sessionCookie(session: Session) {
  return {
    name: SESSION_COOKIE,
    value: signJwt(session, "coinarc-web", 60 * 60 * 24 * 7),
    options: {
      httpOnly: true,
...
export function circleOtpCookieValue(attemptId: string) {
  return {
    name: circleOtpCookie,
    value: attemptId,
    options: {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "strict" as const,
      path: "/api/auth/circle",
      maxAge: 10 * 60,
    },
  }
}
...
export function clearCircleOtpCookie() {
  return {
    name: circleOtpCookie,
    value: "",
    options: { httpOnly: true, path: "/api/auth/circle", maxAge: 0 },
  }
}
